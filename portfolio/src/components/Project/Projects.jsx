...
const PROJECTS = [
  {
    ...
    status: "live",
    description:
      "A real-time analytics console that turns raw event streams into readable, glanceable charts for on-call teams.",
  ...
  },
  {
    ...
    status: "live",
    description:
      "A rate-limited webhook relay that queues, retries, and signs outbound events so downstream services never miss a beat.",
  ...
  },
  {
    ...
    status: "beta",
    description:
      "A CLI that draws a dependency map of any repo in seconds, so onboarding a codebase takes minutes, not days.",
    stack: ["Node.js", "Commander", "Graphviz"],
  },
  {
    slug: "nimbus-cli",
    title: "Nimbus CLI",
    ...
    status: "live",
    description:
      "One command to spin up, seed, and tear down disposable cloud environments for pull-request previews.",
  ...
  },
  {
    ...
    status: "beta",
    description:
      "A lightweight scene graph and renderer for branching interactive fiction, built for writers, not engineers.",
  ...
  },
  {
    ...
    status: "archived",
    description:
      "A two-way sync layer that kept spreadsheets and a Postgres ledger honest before the product moved on.",
  ...
  return (
    <article
      ...
      className={`project-card group relative flex flex-col overflow-hidden rounded-2xl
        ...
        ${variant === "carousel" ? "w-[82%] min-[480px]:w-[62%] sm:w-[46%] flex-shrink-0 snap-center" : "w-full"}
      `}
      style={{ "--accent": meta.color }}
    >
      {/* skeleton "preview" — honest about these being dummy builds: it never finishes loading */}
      <div
      ...
      <div className="flex flex-1 flex-col gap-3 p-5 sm:p-6">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <span
            className="flex-shrink-0 whitespace-nowrap rounded-full border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider"
            style={{
              color: status.color,
        ...
        <div className="mt-2 flex items-center gap-5 border-t border-white/[0.07] pt-4 font-mono text-xs">
          ...
          <a
            ...
            style={{ "--accent": meta.color }}
          >
            <span className="text-white/30">$</span> view --source
          </a>
        </div>
      </div>
    </article>
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-10 max-w-xl sm:mb-14">
          ...
          <p className="mt-4 font-mono text-xs text-white/40">
            ...
            {counts.beta || 0} in beta · {counts.archived || 0} archived
          </p>
        </div>
        ...
        <div className="md:hidden">
          ...
          <div className="mt-5 flex items-center justify-between gap-4">
            <button
              ...
              className="carousel-arrow flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full border border-white/15 text-white/70 transition-colors"
            >
              ←
            </button>
          ...
          {active && (
            <p className="mt-4 text-center font-mono text-[11px] text-white/35">
              [ {pad(activeIndex + 1)} / {pad(featuredFirst.length)} ]{" "}
              {active.slug} — {STATUS_META[active.status].label.toLowerCase()}
            </p>
          )}
        </div>
      </div>
